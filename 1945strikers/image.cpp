...
void image::alphaframeRender(HDC hdc, int destX, int destY, BYTE alpha)
{
		...
		if (_isTrans)//배경색 없애고 출력
		{
			...
			GdiAlphaBlend(hdc, destX, destY, _imageInfo->frameWidth, _imageInfo->frameHeight,
				_blendImage->hMemDC, 0, 0, _imageInfo->currentFrameX * _imageInfo->frameWidth, _imageInfo->currentFrameY * _imageInfo->frameHeight, _blendFunc);
		}
		else//원본 이미지 그대로 출력
		{
			GdiAlphaBlend(hdc, destX, destY, _imageInfo->frameHeight, _imageInfo->frameHeight,
				_imageInfo->hMemDC, 0, 0, _imageInfo->currentFrameX * _imageInfo->frameWidth, _imageInfo->currentFrameY * _imageInfo->frameHeight, _blendFunc);
		}
